...
export function PlayerVehicle({ game }: { game: Game }) {
  ...
  useBeforePhysicsStep(() => {
    ...
    if (translation) {
      const q = chassisRef.current!.rotation();
      const v = chassisRef.current!.linvel();
      const headingRad = Math.atan2(2 * (q.x * q.z + q.w * q.y), 1 - 2 * (q.x * q.x + q.y * q.y));
      game.events.publish('net/localTransform', {
  ...
  // Starting grid: offset each player to the left of the start line by their
  // slot, so multiplayer cars don't spawn inside each other (single-player = 0).
  const GRID_SPACING = 3.2;
  const lateral = game.spawnIndex * GRID_SPACING;
  const spawnX = spawn.x + Math.cos(spawn.headingRad) * lateral;
  const spawnZ = spawn.z - Math.sin(spawn.headingRad) * lateral;
  const spawnY = elevationAt(game.map.terrain, spawnX, spawnZ) + 1.1;
